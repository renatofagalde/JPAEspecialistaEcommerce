...
public class GroupByTest extends EntityManagerTest {
    ...
    public void condicionarAgrupamentoComHaving() {
        //total de vendas dentre as categorias que mais vendem, neste caso acima de 100 para treinar o having
        String jpql = "select cat.nome,sum(ip.precoProduto) from ItemPedido ip join ip.produto pro join pro.categorias cat"+
                " group by cat.id "+
        ...
        lista.forEach(objeto -> System.out.println(MessageFormat.format("Posicao 0 -> {0} e na posicao {1}", objeto[0], objeto[1])));
...
//        Total de vendas por cliente
        String jpql = "select c.nome, sum(ip.precoProduto) from ItemPedido ip " +
                ...
                " where year(p.dataCriacao) = year(current_date) and month(p.dataCriacao) >= (month(current_date) - 3) " + //ultimo mes ultimo 3 meses
                " group by c.id";
